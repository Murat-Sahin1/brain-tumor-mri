...
\vspace{0.5cm}
We utilized the U-Net architecture [@DBLP:journals/corr/RonnebergerFB15] for segmentation, which is a fully convolutional neural network. U-Net comprises two main components: the contracting path (encoder) and the expansive path (decoder). These components are interconnected through skip connections. The encoder consists of 4 blocks, each containing 2 convolutional layers followed by a max pooling layer. The decoder also consists of 4 blocks, with each block containing 1 up-convolutional layer and 2 convolutional layers.
...
```
We were able to receive 70% Dice coefficient with 60% IoU Metric. Also predicted images were less detailed than the other ones, see figure 5. 
\vspace{0.5cm}
...
```
\newpage
\leftline{\large \textbf{3.4\hspace{1cm}Fully Convolutional Network}}
\vspace{0.5cm}
Every other model that we tested were already a fully convolutional network with skip connections. The one we tried is more compact form with 3 blocks with convolution layer and max pooling layer in encoder; and 1 up-convolutional layer and 1 convolutional layer with skip connections.

\newpage
\leftline{\large \textbf{2\hspace{1cm}Results}}
\vspace{0.5cm}
